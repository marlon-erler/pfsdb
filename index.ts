...
export class Util {
    ...
    static logEntryValueActivity(
        entryId: string,
        fieldName: string,
        verb: string,
        values: string[],
    ): void {
        this.logEntryActivity(
            `${entryId}, field ${fieldName}: ${verb} ${
                values.length == 1 ? "value" : `${values.length} values:`
            } ${values.join(", ")}`,
        );
    }
...
export class Database {
    ...
    private async createParentDirectoryForFileOrFail(
        filePath: string[],
    ): Promise<void> {
...
export class Table<F extends string> {
    ...
    private getEntryPathForFieldValue(
        fieldName: F,
        fieldValue: string,
        entryId: string,
    ): string[] {
        const fieldValuePath: string[] = this.getValuePathForField(
            fieldName,
            fieldValue,
        );
    ...
    private getFieldValuePathForEntry(
        entryId: string,
        fieldName: F,
        fieldValue: string,
    ): string[] {
        const fieldPath: string[] = this.getFieldPathForEntry(
            entryId,
            fieldName,
        );
        const valueKey: string = Util.generateValueKey(fieldValue);
    ...
    async getAllEntries(): Promise<string[]> {
        try {
            return await this.database.readDirectoryOrFail(
                this.entryContainerPath,
            );
        } catch {
            return [];
        }
    }
    async getEntriesByFieldValue(
        fieldName: F,
        possibleFieldValues: string[],
    ): Promise<string[]> {
        const allMatchingEntryIds = [] as string[];
        for (const possibleValue of possibleFieldValues) {
            try {
                const valuePath: string[] = this.getValuePathForField(
                    fieldName,
                    possibleValue,
                );
                const matchingEntryIds: string[] =
                    await this.database.readDirectoryOrFail(valuePath);
    ...
    async getValuesForField(entryId: string, fieldName: F): Promise<string[]> {
        try {
            const directoryPath: string[] = this.getFieldPathForEntry(
                entryId,
                fieldName,
            );
            const valueKeys: string[] = await this.database.readDirectoryOrFail(
                directoryPath,
            );
            ...
            for (const key of valueKeys) {
                try {
                    ...
                    const value: string = await this.database.readFileOrFail(
                        valuePath,
                    );
                    values.push(value);
                } catch {}
    ...
    async clearFieldValuesForEntry(
        entryId: string,
        fieldName: F,
    ): Promise<void> {
        const fieldValues: string[] = await this.getValuesForField(
            entryId,
            fieldName,
        );
        await this.removeFieldValuesFromEntry(entryId, fieldName, fieldValues);
    }
    async removeFieldValuesFromEntry(
        entryId: string,
        fieldName: F,
        valuesToRemove: string[],
    ): Promise<void> {
        Util.logEntryValueActivity(
            entryId,
            fieldName,
            "removing",
            valuesToRemove,
        ...
        for (const fieldValue of valuesToRemove) {
            // field
            try {
                const pathInField: string[] = this.getEntryPathForFieldValue(
                    fieldName,
                    fieldValue,
                    entryId,
                );
            ...
            // entry
            try {
                const pathInEntry: string[] = this.getFieldValuePathForEntry(
                    entryId,
                    fieldName,
                    fieldValue,
                );
